...
bootstrapPage('/cocina', () => {
  const kitchenProducts = PRODUCTS.filter((product) => product.area === 'cocina');
    ...
    <div class="col-md-6 col-xl-3">
      <div class="card h-100 shadow-sm order-card">
        <div class="card-body">
          ...
          <p class="text-muted small mb-0">${kitchenProducts.map((product) => product.name).join(', ')}</p>
        </div>
      </div>
    </div>
  `).join('');
